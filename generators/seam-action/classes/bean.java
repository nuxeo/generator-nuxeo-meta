 ...
 *   - ...
 */
@Name("<%= s.dasherize(s.decapitalize(s.titleize(action_name))) %>")
@Scope(ScopeType.EVENT)
public class <%= s.classify(s.camelize(action_name)) %>ActionBean implements Serializable {

    private static final long serialVersionUID = 1L;

    private static final Log log = LogFactory.getLog(<%= s.classify(s.camelize(action_name)) %>ActionBean.class);
    ...
    // clicked
    public String doGet() {
        String message = "Hello from <%= action_name %> : ";

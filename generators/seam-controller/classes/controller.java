 ...
 *
 */
@Name("<%= s.dasherize(s.decapitalize(s.titleize(controller_name))) %>")
@Scope(ScopeType.CONVERSATION)
@AutomaticDocumentBasedInvalidation
public class <%= s.classify(s.camelize(controller_name)) %> extends DocumentContextBoundActionBean implements Serializable {

    private static final long serialVersionUID = 1L;

    private static final Log log = LogFactory.getLog(<%= s.classify(s.camelize(controller_name)) %>.class);
    ...
    @Factory(value="<%= s.dasherize(s.decapitalize(s.titleize(controller_name))) %>UpdateCount", scope=ScopeType.EVENT)
    ...
    @Factory(value="<%= s.dasherize(s.decapitalize(s.titleize(controller_name))) %>Contributors", scope=ScopeType.EVENT)

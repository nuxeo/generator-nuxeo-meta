...
const chromeOptionsArgs = ['--no-sandbox', '--enable-experimental-web-platform-features'];
if (argv['headless']) {
...
exports.config = {
  ...
  //
  specs: argv['features'] ? [argv['features']] : ['./src/test/features/*.feature'],
  // Patterns to exclude.
  exclude: [
  ...
  baseUrl: argv['nuxeoUrl'] ? argv['nuxeoUrl'] : (process.env.NUXEO_URL ? process.env.NUXEO_URL : 'http://localhost:8080/nuxeo/'),
  //
  ...
  cucumberOpts: {
    require: argv['require'] ? [argv['require']] : ['./src/test/step-definitions', './src/test/fixtures'],
